...
exports.create = async (req, res) => {
  ...
  if (result && result._id) {
    ...
    const newBillet = await createNew({ title, content, user_id });
...
exports.read = async (req, res) => {
  ...
  // Object.keys(billets).forEach( async function (key) {
  //   console.table('Key : ' + key + ', Value : ' + billets[key])
  //   const billet_id = billets[key]._id;
  //   console.log(billet_id);
  //   const searchBillets = async (billet_id) => { return await Comment.find({ billet_id: new ObjectId(billet_id) }).then(comment => comment) };

  //   const comments = await searchBillets(billet_id);
  //   billets.newcomment = "ANIMATION CEHVAL"
  // })

  // function searchthis(billet_id) {
  //   return new Promise((resolve, reject) => {
  //     return await Comment.find({ billet_id: new ObjectId(billet_id) }).then(comment => comment)
  //   }
  // }
